...
CREATE OR REPLACE PROCEDURE AwardAlbum(
    ...
    album_name IN ALBUMS.NAME%TYPE,
    award_name IN AWARDS.NAME%TYPE
) IS
BEGIN
    INSERT INTO AWARD_RECEPTIONS(BAND, ALBUM, AWARD)
        VALUES(band_name, album_name, award_name);
END;

CREATE OR REPLACE PROCEDURE AppearOnFestival(
    band_name IN BANDS.NAME%TYPE,
    festival_name IN FESTIVALS.NAME%TYPE,
    festival_date IN FESTIVALS.DATE_START%TYPE
) IS
BEGIN
    INSERT INTO APPEARANCES(BAND, FESTIVAL_NAME, FESTIVAL_DATE_START)
        VALUES(band_name, festival_name, festival_date);
END;
